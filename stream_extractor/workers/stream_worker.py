...
Lógica de disparo:
  1. Monitora stream em fps_idle continuamente
  2. Quando detecta estrutura visual da tabela → entra em ALERT
  3. Em ALERT, usa OCR leve pra checar se "Yellow Cards" apareceu
     (último item da tabela = tabela 100% carregada)
  4. Quando Yellow Cards confirmado → dispara burst capture + OCR completo
"""
...
class StreamWorker:
    ...
    Máquina de estados:
    UNKNOWN → IN_GAME → ALERT (tabela aparecendo) → TABLE_DETECTED (yellow cards visível) → POST_CAPTURE → IN_GAME → ...
    """
    ...
    def __init__(self, stream_config: dict):
        ...
        self._yellow_card_checks = 0  # Tentativas de detectar yellow cards
        self._MAX_YELLOW_CHECKS = 20  # Máximo de checks antes de desistir (~10s a 2fps)
    ...
    def _check_yellow_cards_visible(self, frame) -> bool:
        """
        OCR leve só na parte inferior da tabela para checar se
        'Yellow Cards' já apareceu — indica tabela 100% carregada.
        """
        try:
            import pytesseract
            import numpy as np
            from ocr.extractor import _crop, _to_thresh, detect_content_bounds, adapt_roi

            h, w = frame.shape[:2]
            xs, xe = detect_content_bounds(frame)

            # ROI: parte inferior da coluna de labels (onde fica Yellow Cards)
            ROI_BOTTOM_LABELS = (0.370, 0.820, 0.630, 0.985)
            roi = adapt_roi(ROI_BOTTOM_LABELS, xs, xe, w)
            crop = _crop(frame, roi)
            th = _to_thresh(crop, 2.5)

            text = pytesseract.image_to_string(
                th, config="--oem 3 --psm 6"
            ).lower()

            found = "yellow" in text or "cards" in text or "card" in text
            if found:
                logger.info(f"[{self.stream_id}] 🟡 Yellow Cards detectado — tabela completa!")
            return found
    ...
    def _phase_idle(self, frame) -> bool:
        """
        Fase idle: monitora visualmente se a tela de stats começou a aparecer.
        Não depende mais de tempo de jogo.
        Retorna True se detectou estrutura da tabela.
        """
        _, score = self.detector.is_stats_table_visible(frame)
        if score > 0.45:
            logger.info(f"[{self.stream_id}] ⚡ Tabela detectada (score={score:.2f}), aguardando Yellow Cards...")
            self.detector.set_state(GameState.ALERT)
            self.capture.set_fps(settings.DETECTION["fps_alert"])
            self._yellow_card_checks = 0
            return True
        return False

    def _phase_alert(self, frame) -> bool:
        """
        Fase alerta: tabela está aparecendo, aguarda Yellow Cards
        para confirmar que está 100% carregada antes de capturar.
        Retorna True quando Yellow Cards confirmado.
        """
        # Verifica se a tabela ainda está visível
        detected, score = self.detector.is_stats_table_visible(frame)

        if not detected:
            self._consecutive_detections = 0
            # Se perdeu a tabela, volta para idle
            if self.detector.time_in_state() > 10:
                logger.info(f"[{self.stream_id}] Tabela sumiu, voltando para idle")
        ...
        self._yellow_card_checks += 1

        logger.debug(f"[{self.stream_id}] Aguardando Yellow Cards... check {self._yellow_card_checks}/{self._MAX_YELLOW_CHECKS} | score={score:.3f}")

        # Checa Yellow Cards a cada 2 frames pra não sobrecarregar
        if self._yellow_card_checks % 2 == 0:
            if self._check_yellow_cards_visible(frame):
                self.detector.set_state(GameState.TABLE_DETECTED)
                self._log_event("detection", f"Tabela completa (Yellow Cards visível) | score={score:.3f}")
                return True

        # Timeout: se esperou muito e não viu Yellow Cards, captura assim mesmo
        if self._yellow_card_checks >= self._MAX_YELLOW_CHECKS:
            logger.warning(f"[{self.stream_id}] Timeout aguardando Yellow Cards, capturando assim mesmo")
            self.detector.set_state(GameState.TABLE_DETECTED)
            self._log_event("detection", f"Timeout Yellow Cards, captura forçada | score={score:.3f}")
            return True
