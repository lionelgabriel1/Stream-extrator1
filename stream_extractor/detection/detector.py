...
class TableDetector:
    ...
    # ─── Rejeita tela de pré-jogo (HEAD-TO-HEAD / VS / STARTING IN) ─────────────

    def _is_pregame_screen(self, frame: np.ndarray) -> bool:
        """
        A tela de pré-jogo tem pouquíssimas linhas horizontais na área da tabela
        e uma grande área vazia no centro (VS + logos dos times).
        Se a área central tiver menos de 3 linhas horizontais, é pré-jogo.
        """
        center_roi = self._extract_roi(frame, "stats_table")
        gray = cv2.cvtColor(center_roi, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150)
        lines = cv2.HoughLinesP(
            edges, 1, np.pi/180,
            threshold=60,
            minLineLength=center_roi.shape[1] * 0.2,
            maxLineGap=20
        )
        horizontal = [l for l in lines if abs(l[0][1] - l[0][3]) < 5] if lines is not None else []
        # Tabela real tem muitas linhas (uma por stat), pré-jogo tem poucas
        return len(horizontal) < 4
    ...
    def compute_detection_score(self, frame: np.ndarray) -> Dict[str, float]:
        ...
        # Rejeita imediatamente tela de pré-jogo (VS / HEAD-TO-HEAD / STARTING IN)
        if self._is_pregame_screen(frame):
            return {"dark_overlay": 0.0, "table_lines": 0.0, "colored_bars": 0.0,
                    "white_text": 0.0, "circles": 0.0, "final": 0.0}
